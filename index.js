...
const moduloFMGP = require('./index-FMGP.js');
const moduloGGG = require('./index-GGG.js');

// Ruta de tu compañero (FMGP)
app.get('/samples/FMGP', (req, res) => {
    try {
        const resultado = moduloFMGP.calcularMediaCheaters();
        res.set('Content-Type', 'text/plain');
        res.send(`Media de ${resultado.CampoNumerico} en ${resultado.FiltroPais}: ${resultado.media.toFixed(2)}`);
    } catch (error) {
        console.error("Error:", error);
...
app.get('/samples/GGG', (req, res) => {
    try {
        const resultado = moduloGGG.calcularMediaEdadAtletas();
        res.set('Content-Type', 'text/plain');
        res.send(`Media de edad de los atletas de ${resultado.pais}: ${resultado.media.toFixed(2)} años`);
    } catch (error) {
        console.error("Error:", error);
        res.status(500).send("Error interno del servidor");
    }
...
// Usamos los datos del módulo de tu compañero para la API
const initialData = moduloFMGP.datos;
...
app.get('/', (req, res) => {
    res.json({ 
        message: 'API SOS2526-30 funcionando',
        samples: {
            FMGP: '/samples/FMGP - Media de cheater_report en Spain',
            GGG: '/samples/GGG - Media de edad de atletas de Netherlands'
        },
        api: `${BASE_PATH}`,
        loadData: `${BASE_PATH}/loadInitialData`
    });
...
app.listen(PORT, () => {
    console.log(`Servidor corriendo en puerto ${PORT}`);
    console.log(`- Ruta samples FMGP: /samples/FMGP`);
    console.log(`- Ruta samples GGG: /samples/GGG`);
    console.log(`- API base: ${BASE_PATH}`);
    console.log(`- Cargar datos: ${BASE_PATH}/loadInitialData`);
});
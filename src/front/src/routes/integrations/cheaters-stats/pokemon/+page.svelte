<script>
    ...
    // Token de autenticación (debe coincidir con VALID_CLIENT_TOKENS del proxy)
    ...
    async function initChart() {
        try {
            ...
            // 2. FETCH Pokémon usando el PROXY AUTENTICADO
            console.log('Cargando Pokémon desde proxy autenticado...');
            const proxyRes = await fetch('/api/github-proxy?path=Purukitto/pokemon-data.json/master/pokedex.json', {
                headers: {
            ...
            if (!proxyRes.ok) {
                ...
                throw new Error(`Proxy error: ${errorData.error || proxyRes.status}`);
            }
            
            const pokemonList = await proxyRes.json();
            console.log('Pokémon recibidos:', pokemonList.length);
            ...
            svg.append('text')
                ...
                .style('fill', '#7e22ce')
                .text('⚡ Pokémon API (Proxy Autenticado) vs Cheaters Stats');
...
<div class="container">
    ...
    <p class="subtitle">Proxy Autenticado con Bearer Token</p>
    
    <div id="chart" style="min-height: 600px; width: 100%;"></div>
    
    {#if loading}
        <div class="loading">Cargando datos desde proxy autenticado...</div>
    {/if}
    ...
    <div class="info-note">
        <p><strong>🔐 Autenticación en el proxy:</strong> Bearer Token requerido</p>
        <p><strong>Proxy:</strong> <code>/api/github-proxy</code></p>
        <p><strong>Token válido:</strong> <code>mi-token-secreto-para-demo</code></p>
    </div>
</div>
...
<style>
    ...
    .info-note code { background: #e2e8f0; padding: 0.1rem 0.3rem; border-radius: 4px; }
</style>